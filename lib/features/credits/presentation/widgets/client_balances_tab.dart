...
class ClientBalanceCard extends StatelessWidget {
  ...
  Widget _buildOriginSection(BuildContext context, ClientBalance balance) {
    ...
    return Column(
      children: [
        _buildOriginItem(context, depositTransactions.first),

        // Link para ver historial completo si hay más de 1 transacción
        if (depositTransactions.length > 1) ...[
          const SizedBox(height: 8),
          InkWell(
            onTap: () => _showFullBalanceHistory(context, balance),
            borderRadius: BorderRadius.circular(8),
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    'Ver historial completo (${depositTransactions.length} transacciones)',
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: Theme.of(context).colorScheme.primary,
                      fontWeight: FontWeight.w600,
                    ),
  ...
  /// Muestra el historial completo de transacciones de saldo en un diálogo
  void _showFullBalanceHistory(BuildContext context, ClientBalance balance) {
    final depositTransactions = balance.transactions
        .where((t) => t.type == BalanceTransactionType.deposit)
        .toList();
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Padding(
                      ...
                      child: _buildOriginItem(context, transaction),
                    );
                  },
                ),
              ),
