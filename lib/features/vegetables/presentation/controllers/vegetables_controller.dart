...
class VegetablesController extends GetxController {
  ...
  Future<void> printSale(VegetableSale sale) async {
    final ip = preferencesService.getPrinterIp();
    if (ip == null || ip.isEmpty) {
      safeSnackbar(
        'Impresora no configurada',
        'Configura la IP de la impresora en Facturación > Impresora Térmica',
        snackPosition: SnackPosition.TOP,
      );
      return;
    }
    ...
    try {
      ...
      safeSnackbar('Error al imprimir', e.message, snackPosition: SnackPosition.TOP);
    } catch (e) {
      safeSnackbar('Error al imprimir', e.toString(), snackPosition: SnackPosition.TOP);
    }
  }
