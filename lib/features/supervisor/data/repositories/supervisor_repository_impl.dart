...
class SupervisorRepositoryImpl implements SupervisorRepository {
  ...
  @override
  Future<Either<Failure, List<ProductUpdateTask>>> getPendingTasks({int page = 1, int limit = 20, AssignedRole? assignedRole}) async {
    if (await networkInfo.isConnected) {
      try {
        final remoteTasks = await remoteDataSource.getPendingTasks(page: page, limit: limit, assignedRole: assignedRole);
  ...
  @override
  Future<Either<Failure, List<ProductUpdateTask>>> getCompletedTasks({int page = 1, int limit = 20, AssignedRole? assignedRole}) async {
    if (await networkInfo.isConnected) {
      try {
        final remoteTasks = await remoteDataSource.getCompletedTasks(page: page, limit: limit, assignedRole: assignedRole);
        return Right(remoteTasks);
      } on ServerException catch (e) {
        return Left(ServerFailure(e.message));
      }
